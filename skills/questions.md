...
**When the user confirms a batch has actually been sent to the issuer** — never on the model's own initiative — move those questions into a new batch heading at the end of the document, title and content unchanged but freshly renumbered the same way Active Questions are numbered (independently of their original number), following the structure and field guidance in `templates/questions.md`. Delete the originals from their active category and renumber that category to close the gap. Update the Table of Contents (not the Summary count, which covers only what's still active).
