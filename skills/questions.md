...
Within each section, order questions from highest Priority to lowest (`High` → `Medium` → `Low`), breaking ties by keeping the existing relative order, then renumber `Q-X.1`, `Q-X.2`, ... to match. Question IDs are positional, not stable — re-sort and renumber the whole section whenever a question is added, removed, or an existing question's Priority changes (e.g. during Step 3/4's re-confirmation on a re-run). Update the table of contents and the Summary count section's number ranges to match afterward.

**When the user confirms a batch has actually been sent to the issuer** — never on the model's own initiative — assign it the next sequential batch ID (`QB-01`, `QB-02`, ...) and move those questions into a new batch heading at the end of the document, title and content unchanged but freshly renumbered the same way Active Questions are numbered (independently of their original number), following the structure and field guidance in `templates/questions.md`. Set the `Batch` field on each moved question to this batch's `QB-NN` ID. Delete the originals from their active category and renumber that category to close the gap. Update the Table of Contents (not the Summary count, which covers only what's still active).

`templates/questions.md` defines the required structure — table of contents, section/question numbering (`Q-X.Y`), batch numbering (`QB-NN`), per-question fields, and the closing summary — and carries inline guidance for each part. Follow it exactly.
