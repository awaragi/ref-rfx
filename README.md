...
Each question is written in a `Q-section.question`-numbered structure (e.g. `Q-4.3`) under an Active Questions heading, grouped into topic categories with questions ordered highest-to-lowest Priority within each, a `Batch` field (holding the `QB-NN` ID of the submission round it's assigned to, once one exists), a `Sensitivity` flag (for questions that would reveal competitive positioning if answered publicly), a `Priority` field (`High`/`Medium`/`Low`, blending bid-impact, timeline urgency, and compliance-trap risk — assigned against a High/Medium/Low split target confirmed with the user, defaulting to a soft ~30/40/30 suggestion), and the question itself written as a plain, pasteable paragraph — kept separate from any internal-only rationale, which is preserved in a `> [!NOTE]` callout instead. A table of contents links to every section and question, and once the user confirms a batch was actually sent, it's assigned the next sequential `QB-NN` ID and its questions are moved (not copied) into a new "Submitted batch QB-NN on [date]" heading at the end of the document — title and content unchanged but freshly renumbered within that batch — freeing the active list to keep resorting/renumbering around what's still unsent.
